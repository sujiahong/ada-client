    ...
    takeAction:function(){
        ...
        if(this._lastAction != null && this._lastAction.type == ACTION_CHUPAI){
            if(action != null && action.type != ACTION_PENG && action.type != ACTION_GANG && action.type != ACTION_HU){
                cc.vv.gameMgr.doGuo(this._lastAction.si,this._lastAction.pai);
            }
        }
        ...
        if(action.type == ACTION_CHUPAI){
            //console.log("chupai");
            cc.vv.gameMgr.doChupai(action.si,action.pai);
            return 1.0;
        }
        else if(action.type == ACTION_MOPAI){
            //console.log("mopai");
            cc.vv.gameMgr.doMopai(action.si,action.pai);
            cc.vv.gameMgr.doTurnChange(action.si);
            return 0.5;
        }
        else if(action.type == ACTION_PENG){
            //console.log("peng");
            cc.vv.gameMgr.doPeng(action.si,action.pai);
            cc.vv.gameMgr.doTurnChange(action.si);
            return 1.0;
        }
        else if(action.type == ACTION_GANG){
            //console.log("gang");
            cc.vv.gameMgr.dispatchEvent('hangang_notify',action.si);
            cc.vv.gameMgr.doGang(action.si,action.pai);
            cc.vv.gameMgr.doTurnChange(action.si);
            return 1.0;
        }
        else if(action.type == ACTION_HU){
            //console.log("hu");
            cc.vv.gameMgr.doHu({seatindex:action.si,hupai:action.pai,iszimo:false});
            return 1.5;
        }
    }
